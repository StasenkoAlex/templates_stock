...
{else}
	{$meta_title='Баннера' scope=parent}
{/if}
		...
		<div class="{if !$banners_image->visible}invisible{/if} {if $banners_image->featured}featured{/if} row">
			...
			<div class="move cell"><div class="move_zone"></div></div>
	 		<div class="checkbox cell">
				<input type="checkbox" name="check[]" value="{$banners_image->id}"/>				
			</div>
			<div class="image cell">
				{if $banners_image->image}
				<a href="{url module=BannersImageAdmin id=$banners_image->id return=$smarty.server.REQUEST_URI}"><img src="../{$config->banners_images_dir}{$banners_image->image}" width="30px" /></a>
				{/if}
			</div>
			<div class="name cell">
...
<div id="right_menu">
	...
	<!-- Фильтры -->
	<ul>
		<li {if !$filter}class="selected"{/if}><a href="{url brand_id=null banner_id=null keyword=null page=null filter=null}">Все баннера</a></li>
		<li {if $filter=='visible'}class="selected"{/if}><a href="{url keyword=null brand_id=null banner_id=null page=null filter='visible'}">Активные</a></li>
		<li {if $filter=='hidden'}class="selected"{/if}><a href="{url keyword=null brand_id=null banner_id=null page=null filter='hidden'}">Неактивные</a></li>
	</ul>
	<!-- Фильтры -->
	...
	{if $banners}
	<ul>
		<li {if !$banner->id}class="selected"{/if}><a href="{url banner_id=null brand_id=null}">Все группы</a></li>	
		{foreach $banners as $b}
		<li banner_id="{$b->id}" {if $banner->id == $b->id}class="selected"{else}class="droppable banner"{/if}><a href='{url keyword=null page=null banner_id={$b->id}}'>{$b->name}</a></li>
		{/foreach}
	</ul>
	{/if}
